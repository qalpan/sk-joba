...
app.post('/save-worker', async (req, res) => {
    try {
        ...
        if(!name || !phone || !job || !lat) return res.status(400).send("Missing fields");
...
app.post('/save-goods', async (req, res) => {
    try {
        ...
        if(!name || !product || !price || !phone || !lat) return res.status(400).send("Missing fields");
        const expiresAt = new Date(Date.now() + parseInt(durationHours) * 60 * 60 * 1000);
...
// API: ТАПСЫРЫС САҚТАУ (Растаусыз)
app.post('/save-order', async (req, res) => {
    try {
        const { name, description, phone, lat, lon, device_token } = req.body;
        if(!name || !description || !phone || !lat) return res.status(400).send("Missing fields");
...
// API: КАРТА ҮШІН БАРЛЫҚ ДЕРЕКТЕРДІ АЛУ
...
app.get('/admin/pending', async (req, res) => {
    try {
        ...
        const g = await pool.query(`SELECT id, seller_name as name, product_name as info, phone, 'good' as type, to_char(created_at, 'DD.MM HH24:MI') as date_text, CASE WHEN (expires_at - created_at) > interval '2 hour' THEN '490₸' ELSE '49₸' END as price FROM goods WHERE is_active = FALSE`);
