...
// Мәліметтерді сақтау (Сервер жағындағы валидациямен)
app.post('/save-worker', async (req, res) => {
    const { name, phone, job, lat, lon, durationHours, device_token } = req.body;
    if(!name || !phone || !job || !lat) return res.status(400).json({error: "Барлық жолды толтырыңыз"});
...
app.post('/save-goods', async (req, res) => {
    ...
    if(!name || !product || !phone || !lat) return res.status(400).json({error: "Барлық жолды толтырыңыз"});
...
app.post('/save-order', async (req, res) => {
    ...
    if(!name || !description || !phone || !lat) return res.status(400).json({error: "Барлық жолды толтырыңыз"});
