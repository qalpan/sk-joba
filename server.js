...
// База құрылымы (5-талап үшін уақыты қосылған)
...
// 3, 4, 6 Талаптар: 24 сағаттық сүзгі
...
// 4-Талап: VIP сақтау (белсенді емес күйде)
app.post('/save', async (req, res) => {
    const { name, job, type, tel, email, lat, lon, is_vip, token } = req.body;
    // VIP болса false, тегін болса true
    ...
    } catch (err) { res.status(500).json({ error: "Сақталмады" }); }
});
